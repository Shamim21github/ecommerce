...
class ProductCategory implements JsonSerializable{
	...
	public static function all($criteria=""){
		global $db,$tx;
		$result=$db->query("select id,name,section_id,created_at,updated_at from {$tx}product_categories $criteria");
		$data=[];
	...
	public static function filter($section_id){
		global $db,$tx;
		$result=$db->query("select id,name,section_id,created_at,updated_at from {$tx}product_categories where section_id='$section_id'");
		$data=[];
		while($productcategory=$result->fetch_object()){
			$data[]=$productcategory;
		}
